...
router.get("/getstreams/:user?", (req, res) => {
  if (req.query.list) {
    ...
    streamSchema
      .find()
      .then((streams) => {
        let finalStreams = streams.filter(
          (s) => data[`${s.key}`] !== undefined
        );
        let newStream = [];
        finalStreams.forEach((st) => {
          newStream = [
            ...
            {
              ...st._doc,
              started: data[st.key]["publisher"]["connectCreated"],
            },
          ];
        });
...
router.delete("/delete/:key", authenticate, (req, res) => {
  streamSchema
    .findOneAndDelete({ key: req.params.key })
    .then((data) => {
      if (data === null) return res.status(404).json(false);
